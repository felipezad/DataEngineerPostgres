...
def process_log_file(cur, filepath):
    """
    Description: This function is responsible for listing the log about the msuic in the json files,
    and then executing the ingest process for each json object according to the function
    that performs the transformation to save it to the database.
    ...
    for index, row in df.iterrows():
        ...
        rowTime = pd.to_datetime(row.ts, unit='ms')
        songplay_data = (rowTime, row.userId, row.level, songid, artistid, row.sessionId, row.location, row.userAgent)
